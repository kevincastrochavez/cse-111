...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for i in range(len(symbol_quantity_list)):
        symbol = symbol_quantity_list[i][NAME_INDEX]
        quantity = symbol_quantity_list[i][ATOMIC_MASS_INDEX]

        element_mass = periodic_table[symbol][ATOMIC_MASS_INDEX] * quantity
        mass = mass + element_mass 
